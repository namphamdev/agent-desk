/**
 * Auth HTTP routes — ported from edge/src/auth-routes.ts.
 *
 ...
 * - POST /auth/orgs         — create org + first (admin) membership.
 * - GET  /auth/cli/callback — headless sign-in paste-code page.
 *
 * Exchange/refresh/callback run BEFORE the bearer gate; org routes verify
 * the bearer themselves.
 */
import type { Hono } from "hono";
import { bearerFromHeaders, verifyToken } from "./authenticate";
...
/** Mount auth routes on a Hono app. Uses `any` for app type since Hono's
 * generic type changes when Variables are configured on the parent app. */
export const mountAuthRoutes = (app: Hono, env: Env): void => {
  app.post("/auth/exchange", async (c) => {
    const apiKey = env.WORKOS_API_KEY;
    if (!apiKey) return notConfigured();
    const body = await bodyJson<{ code?: string }>(c.req.raw);
  ...
  app.post("/auth/refresh", async (c) => {
    const apiKey = env.WORKOS_API_KEY;
    if (!apiKey) return notConfigured();
    const body = await bodyJson<{ refreshToken?: string; organizationId?: string }>(c.req.raw);
  ...
  app.get("/auth/orgs", async (c) => {
    ...
    const token = bearerFromHeaders(c.req.raw.headers, url);
  ...
  app.post("/auth/orgs", async (c) => {
    ...
    const caller = token ? await verifyToken(env, token) : undefined;
    if (!caller) return json({ error: "invalid or missing bearer token" }, 401);
    const body = await bodyJson<{ name?: string }>(c.req.raw);
  ...
  app.get("/auth/cli/callback", (c) => {
    return cliCallback(new URL(c.req.url));
  });
};
