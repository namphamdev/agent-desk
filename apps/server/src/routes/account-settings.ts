/**
 * Account settings — encrypted per-user sync blob. Ported from
 * edge/src/account-settings.ts. Uses the filesystem blob store instead of R2.
 */
import type { Env } from "../env";
import { createFilesystemBlobStore } from "../storage/blob-store";

export const MAX_SETTINGS_BYTES = 1 * 1024 * 1024;
...
export const encryptSettings = async (
  ...
  const ciphertextBuffer = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, key, plaintext as any);
...
export const decryptSettings = async (
  ...
  try {
    const plaintextBuffer = await crypto.subtle.decrypt({ name: "AES-GCM", iv: iv as any }, key, ciphertext as any);
...
export const handleAccountSettings = async (
  ...
  if (method === "GET") {
    const encryptedBytes = store.get(r2Key);
    if (!encryptedBytes) {
  ...
  if (bodyBytes.byteLength > MAX_SETTINGS_BYTES) {
    return json({ error: "too_large" }, 413);
  }
  ...
  store.put(r2Key, encryptedBytes);
