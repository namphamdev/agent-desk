...
function mockHandlers() {
  return {
    ...
    getGitDiff: async () => ({ ok: true as const, diff: "" }),
    stageGitFiles: async () => ({ ok: true as const }),
    unstageGitFiles: async () => ({ ok: true as const }),
    commitGit: async () => ({ ok: true as const, hash: "abc" }),
    fetchGit: async () => ({ ok: true as const, summary: "ok" }),
    pushGit: async () => ({ ok: true as const, summary: "ok" }),
